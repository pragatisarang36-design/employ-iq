...
class CopilotAskView(APIView):
    ...
    def post(self, request):
        ...
        conversation, answer, citations = ask_copilot(student=get_or_create_student_profile(request.user), question=serializer.validated_data["question"], role=role, conversation_id=serializer.validated_data.get("conversation_id"))
        return Response({"answer": answer, "citations": citations, "conversation_id": str(conversation.id), "safety_note": "Guidance is grounded only in the cited curated EmployIQ sources."})
