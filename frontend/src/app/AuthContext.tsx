...
// The register endpoint still requires an institution; students never see or type
// it, so the client supplies the deployment's institution on their behalf.
const DEFAULT_INSTITUTION_SLUG = import.meta.env.VITE_DEFAULT_INSTITUTION_SLUG ?? "demo-college";
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = async (data: Record<string, string>) => (await api.post<User>("/v1/auth/register/", { ...data, institution_slug: data.institution_slug || DEFAULT_INSTITUTION_SLUG, role: "student" })).data;
